...
class TestClassifiedSentence(unittest.TestCase):
    ...
    def test_from_dict(self):
        sentence_out = classify.ClassifiedSentence.from_dict(self.url, self.topic, self.sentence_dict_contra)
    ...
    def test_no_arg(self):
        sentence_out = classify.ClassifiedSentence.from_dict(self.url, self.topic, self.sentence_dict_noarg)
        self.assertIsInstance(sentence_out, classify.ClassifiedSentence)
        self.assertEqual(sentence_out.url, self.url)
        # check that the document id is the same as what we expect
        self.assertEqual(sentence_out.doc_id, self.doc_id)
        self.assertEqual(sentence_out.stance_label, classify.STANCE_LABEL.NA)
        self.assertEqual(sentence_out.argument_label, classify.ARGUMENT_LABEL.NO_ARGUMENT)
